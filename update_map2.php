...
if ($nm != 0)
{
	while (($objUsers = mysql_fetch_object($usersResult)) != False)
	{
		...
		if ($nm != 0)
		{
			...
			//$ListeUtil[$num]['ville'] = htmlentities(utf8_decode(str_replace(array("Ã©","Ã","Ã´","Ã¢","Ã®","Ã¨","Ãª"),array("é","à","ô","â","î","è","ê"),$objComprofiler->cb_ville)));
			...
			$ListeUtil[$num]['geoLat'] = $objComprofiler->geoLat;
			$ListeUtil[$num]['geoLng'] = $objComprofiler->geoLng;
			...
			if ($geoLat == NULL && $geoLng == NULL)
			{
				if ($ville != NULL)
				{
					$location = $ListeUtil[$num]['ville'].', '.$ListeUtil[$num]['pays'];
					$url = 'http://maps.google.com/maps/geo?output=xml&q='.$location;
